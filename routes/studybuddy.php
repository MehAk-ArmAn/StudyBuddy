...
use App\Http\Controllers\Admin\StudyBuddyAppController as AdminStudyBuddyAppController;
...
/*
| Internal release tooling. These render the build checklist and platform
| status — material for whoever runs StudyBuddy, not for learners or parents.
| The URLs are kept so existing bookmarks and admin links keep resolving, but
| they now require an admin session instead of being publicly readable.
*/
...
Route::middleware(['auth', 'admin'])
    ...
    ->group(function (): void {
        ...
        /*
        | Apps CMS — the place an admin adds, edits, publishes and retires
        | StudyBuddy learning apps. Writes to studybuddy_mini_app_platforms,
        | which is the same table the public /apps pages read.
        */
        Route::get('/apps', [AdminStudyBuddyAppController::class, 'index'])->name('apps.index');
        Route::get('/apps/create', [AdminStudyBuddyAppController::class, 'create'])->name('apps.create');
        Route::post('/apps', [AdminStudyBuddyAppController::class, 'store'])->name('apps.store');
        Route::post('/apps/reorder', [AdminStudyBuddyAppController::class, 'reorder'])->name('apps.reorder');
        Route::get('/apps/{app}/preview', [StudyBuddyFinalPlatformController::class, 'adminAppPreview'])->name('apps.preview');
        Route::get('/apps/{app}/preview/play', [StudyBuddyFinalPlatformController::class, 'adminWebPlayPreview'])->name('apps.preview.play');
        Route::get('/apps/{app}/preview/build/{path?}', [StudyBuddyWebAppAssetController::class, 'preview'])
            ->where('path', '.*')
            ->name('apps.preview.asset');
        Route::get('/apps/{app}/edit', [AdminStudyBuddyAppController::class, 'edit'])->name('apps.edit');
        Route::put('/apps/{app}', [AdminStudyBuddyAppController::class, 'update'])->name('apps.update');
        Route::patch('/apps/{app}/publish', [AdminStudyBuddyAppController::class, 'togglePublish'])->name('apps.publish');
        Route::patch('/apps/{app}/featured', [AdminStudyBuddyAppController::class, 'toggleFeatured'])->name('apps.featured');
        Route::delete('/apps/{app}', [AdminStudyBuddyAppController::class, 'destroy'])->name('apps.destroy');
