...
class StudyBuddyAdminContentController extends Controller
{
    public function index(): View
    {
        ...
        return view('admin.studybuddy.content-studio.index', compact('pages', 'items'));
    }
    ...
    public function updateApp(Request $request, StudyBuddyAppCatalogItem $app): RedirectResponse
    {
        ...
        // This model belongs to the retired catalogue. Keep the legacy route
        // non-mutating so old bookmarks cannot revive a second Apps system.
        return redirect()
            ->route('admin.control-room.apps.index')
            ->with('status', 'Apps are managed in the Apps workspace. No legacy record was changed.');
    }
