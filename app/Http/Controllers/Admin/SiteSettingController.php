...
class SiteSettingController extends Controller
{
    public function index(): View
    {
        ...
        if (Schema::hasColumn('site_settings', 'sort_order')) $query->orderBy('sort_order');
    ...
    {
        $route = request()->is('admin/control-room*') ? 'admin.control-room.site-settings' : 'admin.site-settings';
        return view('admin.resources.form', ['title'=>'Create Site Setting','route'=>$route,'fields'=>$this->fields(),'item'=>new SiteSetting(),'method'=>'POST']);
    }
    ...
    {
        $route = request()->is('admin/control-room*') ? 'admin.control-room.site-settings' : 'admin.site-settings';
        return view('admin.resources.form', ['title'=>'Edit Site Setting','route'=>$route,'fields'=>$this->fields(),'item'=>$siteSetting,'method'=>'PUT']);
    }
    ...
    {
        return request()->is('admin/control-room*') ? url('/admin/control-room/site-settings') : route('admin.site-settings.index');
    }
    ...
    private function fields(): array
    {
        ...
        foreach (['type', 'group', 'sort_order'] as $column) {
            if (Schema::hasColumn('site_settings', $column)) $fields[] = $column;
        }
        return $fields;
    }
    ...
    {
        $rules = ['key'=>['required','string','max:120'], 'value'=>['nullable','string']];
        if (Schema::hasColumn('site_settings','type')) $rules['type'] = ['nullable','string','max:40'];
        if (Schema::hasColumn('site_settings','group')) $rules['group'] = ['nullable','string','max:80'];
        if (Schema::hasColumn('site_settings','sort_order')) $rules['sort_order'] = ['nullable','integer'];
