    ...
    <div class="sb-control-panel">
        <div class="sb-control-panel-head wide">
            ...
            <div class="sb-control-row-actions">
                @foreach(($quick_links ?? []) as $link)
                    <a href="{{ $link['url'] ?? '#' }}">{{ $link['label'] ?? 'Open' }}</a>
                @endforeach
                @if(isset($route) && Route::has($route . '.create'))
                    <a class="primary" href="{{ route($route . '.create') }}">Create new</a>
                @endif
            </div>
        </div>
        ...
        <div class="sb-control-table-wrap">
            <table class="sb-control-table">
                <thead><tr>
                    @foreach(($fields ?? []) as $field)<th>{{ str($field)->replace('_', ' ')->title() }}</th>@endforeach
                    <th>Actions</th>
                </tr></thead>
                <tbody>
                    @forelse($items as $item)
                        <tr>
                            @foreach(($fields ?? []) as $field)
                                ...
                                @endphp
                                <td title="{{ $value }}">{{ str($value ?? '—')->limit(86) }}</td>
                            @endforeach
                            <td><div class="sb-control-table-actions">
                                @if(isset($route) && Route::has($route . '.edit'))<a href="{{ route($route . '.edit', $item) }}">Edit</a>@endif
                                @if(isset($route) && Route::has($route . '.destroy'))
                                    <form method="POST" action="{{ route($route . '.destroy', $item) }}" onsubmit="return confirm('Delete this item?')">@csrf @method('DELETE')<button type="submit">Delete</button></form>
                                @endif
                            </div></td>
                        </tr>
                    @empty
