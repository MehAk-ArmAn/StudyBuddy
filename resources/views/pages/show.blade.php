...
@section('content')
    @php
        ...
        $planet = $imageUrl($assetBase . 'decor/planets/planet-ringed-lg.png');
    ...
    <section id="top" class="page-hero sb-hero">@if ($planet)<img class="float-planet planet-left" src="{{ $planet }}" alt="" aria-hidden="true">@endif<div class="hero-copy">@if ($page->eyebrow)<p class="eyebrow">{{ $page->eyebrow }}</p>@endif<h1>{!! str_replace('StudyBuddy', '<span>StudyBuddy</span>', e($page->hero_title ?: $page->title)) !!}</h1>@if ($page->hero_subtitle)<p class="hero-subtitle">{{ $page->hero_subtitle }}</p>@endif @if ($page->hero_body)<p class="hero-body">{{ $page->hero_body }}</p>@endif<div class="hero-actions">@if ($page->button_label)<a class="btn btn-primary" href="{{ $page->button_url ?: '#sections' }}">{{ $page->button_label }}</a>@endif @if ($page->secondary_button_label)<a class="btn btn-ghost" href="{{ $page->secondary_button_url ?: route('home') }}">{{ $page->secondary_button_label }}</a>@endif</div></div><div class="hero-art-wrap page-art-wrap"><span class="orbit orbit-one"></span><span class="sparkle s1">✦</span><span class="sparkle s2">★</span><span class="sparkle s3">✧</span>@if ($heroImage)<img class="hero-art" src="{{ $heroImage }}" alt="{{ $page->title }}">@endif</div></section>
