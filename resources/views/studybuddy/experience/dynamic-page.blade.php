...
@section('title', $page->meta_title ?? $page->title ?? 'StudyBuddy')
...
<div class="sbx-page" data-sbx-page="{{ $slug }}">
    <section class="sbx-hero">
        <div class="sbx-hero__copy">
            ...
            <p>{{ $page->subtitle ?? 'A focused place to learn, practice, and make progress.' }}</p>
        ...
        <aside class="sbx-hero__panel">
            <span>{{ $page->hero_badge ?? 'StudyBuddy guide' }}</span>
            <strong>One dashboard. Multiple apps. Connected progress.</strong>
            <p>Plan a short session, choose the right learning tool, and keep each next step easy to understand.</p>
        </aside>
    </section>
    ...
    @foreach($blocks as $block)
        ...
        @elseif(in_array($type, ['cards', 'steps', 'split', 'checklist'], true))
            <section class="sbx-section">
                <div class="sbx-section__head">
                    <p class="sbx-kicker">{{ $blockKickers[$type] }}</p>
