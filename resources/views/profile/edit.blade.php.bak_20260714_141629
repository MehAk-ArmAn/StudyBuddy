    ...
    <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="profile-studio customizer-studio">
            ...
            @foreach(['profile_theme' => 'Profile theme', 'profile_frame' => 'Avatar frame', 'profile_badge' => 'Showcase badge', 'profile_color' => 'Accent colour', 'avatar_shape' => 'Avatar shape'] as $field => $label)
                ...
                <div class="custom-shop-group">
                    ...
                    <div class="custom-shop-grid">
                        @foreach($customizations[$field] as $value => $item)
                            @php
        ...
        <section class="profile-form-card">
            ...
            <div class="visibility-grid">
                ...
                <label><input type="checkbox" name="show_favorite_apps" value="1" @checked(old('show_favorite_apps', $profile['show_favorite_apps'] ?? true))><span>Show favorite apps</span></label>
            </div>
