...
@section('content')
    ...
    @if ($apps)<section id="apps" class="sb-section home-app-preview launcher-preview-strip"><div class="section-heading wide-heading"><div>@if ($apps->eyebrow)<p class="eyebrow">{{ $apps->eyebrow }}</p>@endif<h2>{{ $apps->title }}</h2>@if ($apps->subtitle)<p>{{ $apps->subtitle }}</p>@endif</div>@if ($apps->button_label)<a class="btn btn-ghost" href="{{ $apps->button_url ?: url('/apps') }}">{{ $apps->button_label }}</a>@endif</div><div class="app-preview-row">@foreach ($apps->items->take(4) as $item)<article class="app-card preview-card lively-card">@if ($item->image_path)<img src="{{ $imageUrl($item->image_path) }}" alt="{{ $item->title }}">@endif<h3>{{ $item->title }}</h3>@if ($item->subtitle)<p class="mini-copy">{{ $item->subtitle }}</p>@endif</article>@endforeach</div></section>@endif
    @if ($pages)<section id="page-paths" class="sb-section page-paths-section path-orbit-section"><div class="section-heading">@if ($pages->eyebrow)<p class="eyebrow">{{ $pages->eyebrow }}</p>@endif<h2>{{ $pages->title }}</h2>@if ($pages->subtitle)<p>{{ $pages->subtitle }}</p>@endif</div><div class="path-grid">@foreach ($pages->items as $item)<a class="path-card lively-card" href="{{ $item->button_url ?: '#' }}">@if ($item->image_path)<img src="{{ $imageUrl($item->image_path) }}" alt="{{ $item->title }}">@endif<span>{{ $item->badge_text ?: 'Path' }}</span><h3>{{ $item->title }}</h3>@if ($item->subtitle)<p>{{ $item->subtitle }}</p>@endif</a>@endforeach</div></section>@endif
